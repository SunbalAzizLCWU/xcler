...
export default async function ShopifyDevelopmentPage({
  ...
  const features = t.raw("coreFeatures") as FeatureItem[];
  ...
  const faqSchema = faqItems.length
    ? getFaqSchema(resolvedLocale, faqItems, "/services/shopify-development-germany")
    : null;
  ...
  return (
    <>
      ...
      {faqSchema ? (
        <JsonLd id={`faq-shopify-development-${locale}`} data={faqSchema} />
      ) : null}
      <section className="section-padding pt-32 relative overflow-hidden">
        ...
        <div className="container-custom relative z-10">
          <div className="xl:grid xl:grid-cols-[minmax(0,1fr)_320px] xl:gap-10 xl:items-start">
            ...
            <div className="mt-8 rounded-3xl border border-stone/15 bg-gradient-to-br from-white/85 via-white/70 to-terracotta/10 p-8 shadow-[0_20px_60px_-40px_rgba(13,13,13,0.45)] backdrop-blur-sm dark:border-stone-dark/20 dark:from-richblack/40 dark:via-richblack/35 dark:to-richblack/20 md:p-12">
              <div className="flex items-center gap-4 mb-6">
                <div className="line-decoration" />
                <span className="font-mono text-xs tracking-[0.3em] text-richblack/40 dark:text-cream/40 uppercase">
                  {locale === "de" ? "Leistung / Shopify" : "Service / Shopify"}
                </span>
              </div>
              ...
              <div className="mt-10 grid grid-cols-1 gap-4 sm:grid-cols-3">
                <div className="rounded-2xl border border-stone/15 bg-white/70 p-4 dark:border-stone-dark/20 dark:bg-richblack/30">
                  <p className="font-mono text-[11px] tracking-[0.2em] uppercase text-richblack/40 dark:text-cream/40">{locale === "de" ? "Checkout" : "Checkout"}</p>
                  <p className="mt-2 font-heading text-lg">{locale === "de" ? "Conversion-fokussiert" : "Conversion-driven"}</p>
                </div>
                <div className="rounded-2xl border border-stone/15 bg-white/70 p-4 dark:border-stone-dark/20 dark:bg-richblack/30">
                  <p className="font-mono text-[11px] tracking-[0.2em] uppercase text-richblack/40 dark:text-cream/40">{locale === "de" ? "Automation" : "Automation"}</p>
                  <p className="mt-2 font-heading text-lg">{locale === "de" ? "API-orchestriert" : "API-orchestrated"}</p>
                </div>
                <div className="rounded-2xl border border-stone/15 bg-white/70 p-4 dark:border-stone-dark/20 dark:bg-richblack/30">
                  <p className="font-mono text-[11px] tracking-[0.2em] uppercase text-richblack/40 dark:text-cream/40">{locale === "de" ? "Skalierung" : "Scale"}</p>
                  <p className="mt-2 font-heading text-lg">{locale === "de" ? "DACH-ready" : "DACH-ready"}</p>
                </div>
              </div>
            </div>
              </AnimatedSection>
            ...
            <div className="mt-20 grid grid-cols-1 gap-8 lg:grid-cols-[220px_minmax(0,1fr)]">
              <div className="lg:sticky lg:top-28 h-max rounded-2xl border border-stone/15 bg-white/70 p-5 dark:border-stone-dark/20 dark:bg-richblack/30">
                <p className="font-mono text-[11px] tracking-[0.25em] uppercase text-richblack/40 dark:text-cream/40">{locale === "de" ? "System-Blueprint" : "System Blueprint"}</p>
                <p className="mt-3 text-sm leading-relaxed text-richblack/60 dark:text-cream/60">{locale === "de" ? "Technischer Kontext fuer Headless-Architektur, Checkout und Betriebsmodell." : "Technical context for headless architecture, checkout, and operating model."}</p>
              </div>
            ...
            <div className="mt-20 relative overflow-hidden rounded-3xl bg-richblack p-8 md:p-10 text-cream shadow-[0_30px_80px_-45px_rgba(13,13,13,0.8)]">
              ...
              <div className="relative">
                <h2 className="font-heading text-2xl md:text-3xl font-semibold">{locale === "de" ? "Automation-Architektur" : "Automation Architecture"}</h2>
                <p className="mt-4 text-cream/75 leading-relaxed">{automationArchitecture}</p>
              </div>
            </div>
              </AnimatedSection>
            ...
            <div className="mt-20 rounded-3xl border border-stone/15 bg-white/80 p-8 md:p-10 shadow-[0_20px_55px_-40px_rgba(13,13,13,0.5)] dark:border-stone-dark/20 dark:bg-richblack/35">
              <div className="flex items-end justify-between gap-6 border-b border-stone/15 pb-6 dark:border-stone-dark/20">
                <h2 className="font-heading text-3xl md:text-4xl font-semibold">{faqTitle}</h2>
            ...
            <div className="mt-20 relative overflow-hidden rounded-3xl bg-richblack p-10 text-cream text-center shadow-[0_35px_80px_-45px_rgba(13,13,13,0.85)]">
              ...
              <h2 className="font-heading text-3xl font-bold">{locale === "de" ? "Sie wollen ein Shopify-System, das langfristig skaliert?" : "Need a Shopify system that scales long-term?"}</h2>
              <p className="mt-3 text-cream/70 max-w-2xl mx-auto">{locale === "de" ? "Wir verbinden Conversion, Betriebsstabilitaet und Automatisierung in einer belastbaren Architektur." : "We combine conversion, operational resilience, and automation into one robust architecture."}</p>
              <Link
                href="/contact"
              ...
              <RelatedServices
                ...
                items={[
                  {
                    href: "/services/wordpress-development-germany",
                    label: locale === "de" ? "WordPress-Agentur Deutschland" : "WordPress agency Germany",
                  },
                  {
                    href: "/services/wordpress-shopify",
                  ...
                  },
                  {
                    ...
                    label: locale === "de" ? "n8n- & Make.com-Agentur" : "n8n & Make.com agency",
                  },
                ]}
              />
            </div>
